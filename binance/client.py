...
#             logging.exception(f'Status error in requesting {method} {uri.split(".com")[1]} {kwargs}: {response.text}', exc_info=False)
    ...
    def request(self, method, path, signed=True, force_params=False, data={}, **kwargs):
        return self._request(method, uri="https://api.binance.com"+path, signed=signed, force_params=force_params, data=data, **kwargs)
